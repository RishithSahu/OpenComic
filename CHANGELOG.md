...
- Fix background thumbnail generation for the library grid competing with the reader for the same thread while a book is open. Generating a PDF's thumbnail means pdf.js rasterising a page on this same renderer thread — for a library scan discovering PDFs while the user is mid-scroll in an unrelated book, that stole frame budget from the same throttle above. Grid thumbnails are not visible while reading regardless of whether they finish now or a few seconds later, so this work now waits for the reading session to end (capped at two minutes, so leaving a book open indefinitely cannot starve it forever).
